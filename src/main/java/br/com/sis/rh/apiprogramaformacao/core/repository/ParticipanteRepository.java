...
public interface ParticipanteRepository extends JpaRepository<Participante, String> {
	...
	@Query(value = "select count(p) from TB_PARTICIPANTE p " + "JOIN TB_CANDIDATO TC ON TC = p.candidato "
			+ "JOIN TB_PROGRAMA prog ON prog = p.programa  "
			+ "JOIN TB_PROCESSO_SELETIVO processo ON processo = prog.processoSeletivo " + "WHERE p.status = 'ATIVO' "
			+ "AND processo.nome = ?1 " + "AND prog.nomeTurma = ?2", nativeQuery = true)
	...
	@Query(value = "SELECT count(p) FROM TB_PARTICIPANTE p " + "JOIN TB_CANDIDATO TC ON TC = p.candidato "
			...
			+ "WHERE p.statusEfetivado = 'EFETIVADO' " + "AND processo.nome = ?1 " + "AND prog.nomeTurma = ?2", nativeQuery = true)
	...
	@Query(value = "SELECT COUNT(p) FROM TB_PARTICIPANTE p WHERE p.status = 'ATIVO'", nativeQuery = true)
	Integer totalParticipantesAtivos();

	// Método que devolve a contagem total de participantes com status EFETIVADO
	@Query(value = "SELECT COUNT(p) FROM TB_PARTICIPANTE p WHERE p.statusEfetivado = 'EFETIVADO'", nativeQuery = true)
